...
import { archiveDashboardTab, deleteDashboardTab } from "@/lib/dashboard-store";
...
export async function DELETE(request: Request, context: { params: Promise<{ tabId: string }> }) {
  const parsed = z.object({ resourceId: z.string() }).safeParse(await request.json());
  if (!parsed.success) return Response.json({ error: parsed.error.message }, { status: 400 });
  const scope = await resolveUserScope(request.headers, parsed.data.resourceId);
  if (!scope.ok) return scope.response;
  const { tabId } = await context.params;
  return Response.json(await deleteDashboardTab(scope.scope.resourceId, tabId));
}
